...
document.addEventListener('click', async (event) => {
  ...
  if (event.target.closest('[data-save-calendar-day]')) { const context = state.availabilityContext; const timezone = value('#calendar-day-timezone'); const location = timezoneLabel(timezone); const selectedTimes = [...document.querySelectorAll('#time-picker-content .time-check input:checked')].map((input) => input.value); try { const path = context.type === 'manager' ? 'managers' : 'candidates'; await API.request(`/hr/${path}/${context.ownerId}/slots/day`, { method: 'PUT', body: JSON.stringify({ date: context.selectedDate, timezone, location, selectedTimes }) }); modal('#time-picker-modal').hide(); window.alert('當天時區與可用時段已儲存。'); await loadAvailabilityCalendar(); } catch (error) { notify(error.message, 'danger'); } return; }
