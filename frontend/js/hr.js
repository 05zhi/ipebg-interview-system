...
async function loadInterviews() {
  ...
  try { const query = new URLSearchParams({ search: value('#interview-search'), status: value('#interview-status') }); const data = await API.request(`/hr/interviews?${query}`); state.interviews = data.interviews; state.interviewPage = 1; renderInterviews(); }
  catch (error) { notify(error.message, 'danger'); }
}
...
document.querySelector('#password-form').addEventListener('submit', async (event) => { event.preventDefault(); try { await API.request('/auth/password', { method: 'PATCH', body: JSON.stringify({ currentPassword: value('#current-password'), newPassword: value('#new-password') }) }); event.target.reset(); notify('密碼已更新。'); } catch (error) { notify(error.message, 'danger'); } });
