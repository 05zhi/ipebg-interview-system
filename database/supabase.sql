...
-- Fresh Supabase PostgreSQL schema for Administrator and HR only.
...
create trigger hr_accounts_set_updated_at before update on public.hr_accounts
for each row execute function public.set_updated_at();
...
begin
  ...
  if target_interview.status <> 'cancelled' and exists (
    select 1
      from public.interview_managers im
      join public.interviews i on i.id = im.interview_id
     where im.manager_id = new.manager_id
       and im.interview_id <> new.interview_id
       and i.status <> 'cancelled'
       and tstzrange(i.starts_at, i.ends_at, '[)') &&
           tstzrange(target_interview.starts_at, target_interview.ends_at, '[)')
  ) then
    raise exception 'Manager already has an interview during this time.';
  end if;
  return new;
end;
$$;

create trigger interview_managers_prevent_overlap
before insert or update on public.interview_managers
...
-- First administrator for a fresh database:
-- insert into public.users (username, password_hash, role)
-- values ('admin', crypt('ChangeThisPassword!', gen_salt('bf', 12)), 'administrator');
